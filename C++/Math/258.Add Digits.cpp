/*
对给定数n，每个位求和，直到结果为一位数
*/
/*
思路：另一个方法比较简单，可以举例说明一下。假设输入的数字是一个5位数字num，则num的各位分别为a、b、c、d、e。
...
因为最后结果在1-9之间，得到9之后将不会再对各位进行相加，
因此不会出现结果为0的情况。因为 (x + y) % z = (x % z + y % z) % z，
...
public:
	int addDigits(int num) {
        return num!=0? num-9*((num-1)/9) : 0;
    }
};
int main()
{
	Solution s;
	cout<<s.addDigits(12345)<<endl;
}
